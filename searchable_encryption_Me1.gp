...
keyword_encrypt(keyword,public_key,Ell,Z,k,limit)=
{
  local(r,A,B,C,peks);
  r=random(limit);
  A=Me_mul_1(Ell,public_key[1,],r,Z,k);
  B=hash1(keyword);
  C=Me_mul_1(Ell,public_key[2,],r,Z,k);

  peks=matrix(2,2);
  peks[1,]=A;
  peks[2,]=elladd(Ell,B,C);
  return(peks);
}

test(peks,trapdoor,Ell)=
{
  local(check);
  check=elladd(Ell,trapdoor,peks[1,]);
  if(ellsub(Ell,check,peks[2,])[1]==0,
		return(1),return(0)
  );
}
...
*/
  ...
  for(i=1,n,
    ...
    for(j=1,n,
      data[i,j]=-1;
      data[i,j]=test(peks,trapdoor[j,],E);
    );
  );
  data
